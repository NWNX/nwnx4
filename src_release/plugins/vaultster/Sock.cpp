...
bool CSock::SendFile (char* File, BOOL bCompress)
{
   ...
   Log ("o Sending data part of the file (%d bytes).\n", actualDataSize);
	...
	while (iSendData < actualDataSize)
	{
      // send the data to the server
		...
		if (nSend == SOCKET_ERROR) {
			int error = WSAGetLastError ();
			Log ("o WinSock error %d!!\n", error);
			return false;
		}
		else if (TIMEOUT (nSend)) {
			Log ("o Time out during transmission.\n");
			return false;
		}
		/*
		else
			Log ("o Send %d bytes of data.\n", nSend);
		*/
		iSendData += nSend;
	}
